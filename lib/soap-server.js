...
SoapServer.prototype.castArgument = function(inputName, requestArgument, options){
	...
	switch(options.type){
		case 'object':
			...
			console.log('reqArg:', requestArgument);
			for(var requestObjectAttribute in requestArgument){

				if (requestArgument[requestObjectAttribute][0]) {
...
SoapServer.prototype.handleOperationRequest = function (service, soapRequest) {
	...
	var operationRequest = soapRequest['soap:Envelope']['soap:Body'][0];
		...
		req.on('end', function () {
			...
			parseXml(body, function (err, soapRequest) {
				...
				try{
					...
					if(exception.name != 'SoapServiceException' && exception.name != 'SoapServerException')
						throw exception;
...
SoapServer.prototype.getWsdl = function(host, service){
	...
	//wsdl += '   xmlns:tns="http://www.examples.com/wsdl/HelloService.wsdl"\n';
	...
	for(operationName in service.operations){
		...
		wsdl += '             <soap:body encodingStyle="http://schemas.xmlsoap.org/soap/encoding/" namespace="urn:examples:helloservice" use="encoded"/>';
		wsdl += '			</input>\n';
